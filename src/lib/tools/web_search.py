...
class ResultDict(dict):
  ...
  Attributes:
    ...
    content (str): The page content (if crawling is enabled).
  """

  def __init__(self, title: str, description: str, url: str, content: str = None):
    self.title = title
...
class Client(BaseClient):
  ...
  def fetch_content(self, url: str, max_size: int = 500) -> str:
    """
    Fetches content from a URL.

    Args:
      url (str): The URL to fetch.
      max_size (int): Maximum content size in characters.

    Returns:
      str: The page content (plain text) or empty string if failed.
    """
    if isinstance(self.proxies, str):
      proxy = self.proxies
    elif isinstance(self.proxies, list):
    ...
    with httpx.Client(proxy=proxy, timeout=10.0) as http:
...
class AsyncClient(BaseClient):
  ...
  async def fetch_content(self, url: str, max_size: int = 500) -> str:
    """
    ...
    if isinstance(self.default_user_agents, str):
      ...
      ua = None
    headers = {'User-Agent': ua} if ua else None
    async with httpx.AsyncClient(proxies=proxy, timeout=10.0, http2=True) as http:
...
def parse_page(html: Union[str, bytes]) -> List[ResultDict]:
  ...
  for section in sections:
    ...
    try:
      ...
      print(f"Unable to parse the page.\n{e}")
  return results


def ddg_search(query: str, crawler: bool = True, max_content_size: int = 500) -> list:
  """
  Make a web search using DuckDuckGo.

  Args:
    query (str): The query to search for.
    crawler (bool): Whether to fetch page content for each result.
    max_content_size (int): Maximum size of content to fetch per page.

  Returns:
    list: A list of top 3 results that contains titles, urls, descriptions, and optionally content.
  """
  client = Client()
  ...
  if crawler:
    for result in results:
      result.content = client.fetch_content(result.url, max_content_size)
